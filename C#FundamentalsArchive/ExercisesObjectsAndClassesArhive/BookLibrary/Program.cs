...
namespace BookLibrary
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            foreach (var item in books.Select(x => x.Author).Distinct().Select(a=>new {Author=a, Price=books.Where(b=>b.Author==a).Sum(p=>p.Price)}).OrderByDescending(ai=>ai.Price).ThenBy(a=>a.Author).ToList())
            {
