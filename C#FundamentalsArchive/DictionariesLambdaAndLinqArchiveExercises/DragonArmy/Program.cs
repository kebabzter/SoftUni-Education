    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            for (int i = 0; i < n; i++)
            {
                ...
                {
                    dragonArmy[type][name] = damage + " " + health + " " + armor;
                }
                if ((dragonArmy.ContainsKey(type) && (!dragonArmy[type].ContainsKey(name))))
                {
                    dragonArmy[type].Add(name, damage + " " + health + " " + armor);
                }
                if (!dragonArmy.ContainsKey(type))
                {
                    dragonArmy.Add(type, new SortedDictionary<string, string>());
                    dragonArmy[type].Add(name, damage + " " + health + " " + armor);
                }
            }
            foreach (var item in dragonArmy)
            {
                int damagePoint = 0;
                int healthPoint = 0;
                int armorPoint = 0;
